...
use kassi_db::diesel_async::{AsyncPgConnection, RunQueryDsl};
use kassi_db::models::{
...
use super::shared::{self, AssetEmbed, DepositAddressEmbed, NetworkEmbed};
...
struct CreateRefundParams<'a> {
    deposit_address: &'a DepositAddress,
...
async fn create_refund(
    ...
    let ledger_entry = kassi_db::queries::insert_ledger_entry(
        ...
        NewLedgerEntry {
            id: &le_id,
            deposit_address_id: &params.deposit_address.id,
...
fn build_refund_response(
    ...
    deposit_address_embed: DepositAddressEmbed,
    asset: &Asset,
    network: &Network,
) -> RefundResponse {
    RefundResponse {
        id: ledger_entry.id,
        deposit_address: deposit_address_embed,
...
pub fn routes() -> Router<AppState> {
    Router::new()
        .route("/payment-intents/{id}/refund", post(refund_payment_intent))
        .route(
...
async fn refund_payment_intent(
    ...
    let asset = kassi_db::queries::get_asset_by_id(&mut conn, &quote.asset_id)
        .await?
        .ok_or_else(|| ServerError::BadRequest(format!("asset '{}' not found.", quote.asset_id)))?;

    // load deposit address (unscoped since we already verified ownership via the PI)
    let deposit_address =
        kassi_db::queries::get_deposit_address_unscoped(&mut conn, &pi.deposit_address_id)
            .await?
    ...
    Ok(ApiSuccess::created(build_refund_response(
        ledger_entry,
        DepositAddressEmbed::from_parts(deposit_address.id, dep_address_str),
        &asset,
        &network,
    )))
}
...
async fn refund_deposit_address(
    ...
    // resolve asset and network from the most recent confirmed deposit (single query)
    ...
    let (ledger_entry, _job) = create_refund(
        ...
        CreateRefundParams {
            ...
            asset_id: &deposit_entry.asset_id,
            network_id: &deposit_entry.network_id,
            amount,
            destination,
            reason: body.reason.as_deref(),
        },
    ...
    let asset =
        kassi_db::queries::load_assets_by_ids(&mut conn, &[deposit_entry.asset_id.as_str()])
            ...
            .into_iter()
            .next()
            .ok_or_else(|| ServerError::BadRequest("asset not found.".into()))?;
...
async fn list_refunds(
    ...
    let next_page = if has_next {
        rows.last()
            .map(|(le, _)| shared::encode_cursor(&le.created_at, &le.id))
    } else {
        None
    };
    ...
    let data: Vec<RefundResponse> = rows
        ...
        .filter_map(|(le, da)| {
            ...
            Some(build_refund_response(
                le,
                DepositAddressEmbed::from_parts(da.id, addr.to_string()),
                asset,
                network,
            ))
        })
