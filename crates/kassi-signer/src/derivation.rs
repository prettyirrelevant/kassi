...
pub(crate) fn derive_evm_signer(
    ...
    PrivateKeySigner::from_slice(&key_bytes).map_err(|e| SignerError::Signing(e.to_string()))
}
...
pub(crate) fn derive_solana_keypair(
    ...
    let chain_id_u32 = u32::try_from(chain_id).map_err(|_| {
        SignerError::Signing("chain_id exceeds u32 range for SLIP-0010 path".into())
    })?;
    let path = [44, chain_id_u32, 0, 0, index];
    let key_bytes = slip10_derive_ed25519(seed, &path);
    keypair_from_seed(&key_bytes).map_err(|e| SignerError::Signing(e.to_string()))
}
